...
import qualified Data.Generics.Internal.Profunctor.Lens as P
import Data.Coerce

import Optics.Core hiding (coerced)
import Optics.Internal.Optic hiding (QuoteType)
...
instance
  ...
  position' = Optic (P.ravel (P.repLens . coerced @cs @cs . glens @(HasTotalPositionPSym i)))
  {-# INLINE position' #-}
...
instance
  ...
  position0 = Optic (P.ravel (P.repLens . coerced @(CRep s) @(CRep t) . glens @(HasTotalPositionPSym i)))
  {-# INLINE position0 #-}
